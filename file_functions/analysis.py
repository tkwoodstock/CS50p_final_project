...
def analysis_countries(data: list):
    ...
    try:
        ...
        print("Thank you for using my CS50p final project: 'Countries',\nby Tyler Woodstock\n")
        return 0
        #return to file
...
def format(countries):
    ...
    for i in countries:
        temp = {
            "Country": i["Country"],
            "GDP ($bn)": i["GDP"],
            "Population (million)": i["Population"],
            "Male Life Expectancy": i["Life Expectancy"]["Male"],
            "Female Life Expectancy": i["Life Expectancy"]["Female"],
            "CO2 (Mt)": i["CO2"],
            }
        results.append(temp)
...
def visualise(countries: list, field):
    ...
    #for GDP and CO2
        ...
        axs[1].bar(x_axis , subplot_list , color = "forestgreen")
...
def write_to_csv(countries, field):
    with open(f"user_country_data.csv" , "w") as file:
        ls = ["Country" , "GDP ($bn)" , "Population (million)" , "Male Life Expectancy" , "Female Life Expectancy" , "CO2 (Mt)"]
